...
GITHUB_CONFIG = {
    ...
    'REPO_OWNER': 'opennews',
    'REPO_NAME': '2020-microloans-journalists',
    'TARGET_FILE': 'docs/data/data.json',
    'TARGET_BRANCHES': ['master',],# choose one or more branches
}
...
GOOGLE_SPREADSHEET_KEY = '1g4COrzaZlGyHs7wLPrDC5oPOF7E6AqV7PzGah2GU-GY'

# pull data from a named worksheet, or leave blank to assume first worksheet
GOOGLE_SPREADSHEET_SHEETNAME = 'Public: Approved Lender Display'
...
def transform_data(data):
    ...
    * ensures that all variables going into the JSON are strings
    ...
    def _transform_response_item(item, skip=False):
        ...
        # use `skip` flag to ignore rows with blank Name
        if 'Name' in _transformed_item:
            if not _transformed_item['Name']:
                skip = True
